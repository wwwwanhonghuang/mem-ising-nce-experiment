...
namespace snnlib
{
    ...
    void SNNNetwork::evolve_states(int t, double dt, std::shared_ptr<snnlib::RecorderFacade> recorder_facade){
        static const std::vector<decltype(neurons)::value_type> neuron_vector(neurons.begin(), neurons.end());
        static const std::vector<decltype(connections)::value_type> connection_vector(connections.begin(), connections.end());
        ...
        #pragma omp parallel for
        for(size_t connection_vector_index = 0; connection_vector_index < connection_vector.size(); connection_vector_index++){
	    auto& connection_record_item = connection_vector[connection_vector_index];
            if(recorder_facade){
