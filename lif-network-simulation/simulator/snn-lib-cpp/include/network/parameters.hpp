...
namespace snnlib
{
    struct BaseParameters: public std::enable_shared_from_this<snnlib::BaseParameters>{
        public:
        std::unordered_map<std::string, int> parameter_index_map;
        std::unordered_map<std::string, double> parameter_value_map;

        std::shared_ptr<snnlib::BaseParameters> 
        ...
        double get(const std::string& parameter_name){
    ...
    struct SynapseParameters: public snnlib::BaseParameters
    {
        
    };
    
    struct SNNNeuronParameters : public snnlib::BaseParameters{
